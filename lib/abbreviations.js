...
export const MAP = [
  // SMSTS
  {
    ...
    aliases: [
      ...
      "site managers safety training scheme", // people often say "managers"
      "site manager safety training scheme",
      "site management safety", // shortened
      "smsts course",
    ],
  },
  {
  ...
  // SSSTS
  {
    ...
    aliases: [
      ...
      "site supervisor safety training scheme", // singular
  ...
  // HSA
  {
    ...
    aliases: [
      ...
      "health and safety",         // extra coverage
      "health & safety",           // extra coverage
      "h&s",
      "hsa course",
  ...
  // TWS (doar standard; dacă NU vrei refresher pentru TWS, nu îl adăuga)
  {
    family: "TWS",
    aliases: [
      "tws",
      ...
      "temporary work supervisor", // typo frecvent
      "tw supervisor",
      "tws course",
    ],
  },
  ...
  // IOSH (generic – acoperă Managing/Working Safely ca umbrelă; ajustează dacă vrei diferențiere)
  {
    family: "IOSH",
    aliases: [
      "iosh",
...
export function expandKeywords(term) {
  ...
  // fallback pe acronime
  if (!family) {
    const acrHits = ["smsts","sssts","twc","tws","hsa","nebosh","iosh","iema","mhfa"];
    const hit = acrHits.find(a => q.includes(a));
    if (hit) {
      const mapAcr = { smsts:"SMSTS", sssts:"SSSTS", twc:"TWC", tws:"TWS", hsa:"HSA", nebosh:"NEBOSH", iosh:"IOSH", iema:"IEMA", mhfa:"MHFA" };
      family = mapAcr[hit];
    }
  }

  // HSA: dacă apare "health and safety" dar nu "nebosh"/"iosh", mapează la HSA
  if (!family) {
    if (/\bhealth(?:\s*&|\s*and)?\s*safety\b/.test(q) && !/\bnebosh\b|\biosh\b/.test(q)) {
      family = "HSA";
    }
  }

  // NEBOSH ambiguu -> "NEBOSH" (umbrela)
...
export function isCourseMatchByFamily(name, family) {
  ...
  const hasTWC = () => n.includes("temporary works coordinator") || n.includes("temporary works co-ordinator") || n.includes("temporary works co ordinator");
...
export function isRefresherMatch(name, refresher) {
  ...
  const n = (name || "").toLowerCase();
  const hasRefresher = /\brefresher\b/.test(n) || /\brenewal\b/.test(n) || /\bupdate\b/.test(n) || /\brefresh\b/.test(n);
