...
if (/\b(this\s*year|for\s+the\s*year|entire\s*year|whole\s*year)\b/.test(q)) {
  const y = NOW.getUTCFullYear();
  return { start: utcDate(y, 0, 1), end: utcDate(y, 11, 31), label: "this year" };
}

// rest of this year (de azi până pe 31 Dec)
if (/\b(rest\s+of\s+(the\s*)?year|till\s+end\s+of\s+year|until\s+end\s+of\s+year)\b/.test(q)) {
  const y = NOW.getUTCFullYear();
  return { start: NOW, end: utcDate(y, 11, 31), label: "rest of this year" };
}

// all year / whole year (interpretez ca 12 luni de azi)
if (/\b(all\s*year|whole\s*year|for\s+all\s+year)\b/.test(q)) {
  const e = utcDate(NOW.getUTCFullYear() + 1, NOW.getUTCMonth(), NOW.getUTCDate());
  return { start: NOW, end: e, label: "next 12 months" };
}

// next year (1 Jan anul viitor – 31 Dec anul viitor)
if (/\bnext\s*year\b/.test(q)) {
  const y = NOW.getUTCFullYear() + 1;
  return { start: utcDate(y, 0, 1), end: utcDate(y, 11, 31), label: "next year" };
}
